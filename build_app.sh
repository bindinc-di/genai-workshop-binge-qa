...
# run locally
# docker run -it --rm  --env-file ".env" -p 8501:8501 $APP_NAME
docker tag $APP_NAME gcr.io/$PROJECT_ID/$APP_NAME
docker push gcr.io/$PROJECT_ID/$APP_NAME
gcloud config set project $PROJECT_ID
gcloud run deploy $APP_NAME --image gcr.io/$PROJECT_ID/$APP_NAME --platform managed --region $REGION \
    --allow-unauthenticated --timeout=120 --port=8501 \
    --set-env-vars "SEARCH_BASE_URL=https://binge-qa-api-iobao7k2yq-ew.a.run.app"
